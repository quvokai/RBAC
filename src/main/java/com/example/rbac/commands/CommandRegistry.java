    ...
    private static void registerAsyncCommands(CommandParser parser) {
        parser.registerCommand("report-users-async", "Запустить генерацию отчёта по пользователям в фоне", (scanner, system) -> {
            ...
            Future<String> future = system.getBackgroundExecutor().submit(() -> {
                Thread.sleep(500); 
                return ReportGenerator.generateUserReportParallel(system.getUserManager(), system.getAssignmentManager());
            });
            ...
            system.getAuditLog().log("REPORT_ASYNC_QUEUED", currentUser, "ReportGenerator", "Задача генерации отчёта поставлена в очередь");
        });
        ...
        parser.registerCommand("save-async", "Сохранить данные в файл в фоновом потоке", (scanner, system) -> {
            ...
            Future<String> future = system.getBackgroundExecutor().submit(() -> {
                ...
                String reportData = system.generateStatistics() + "\n" + 
                                    ReportGenerator.generateUserReportParallel(system.getUserManager(), system.getAssignmentManager());
                
                ReportGenerator.exportToFile(reportData, filename);

                return String.format("Сохранено в '%s': %d пользователей, %d ролей, %d назначений.", filename, users, roles, assigns);
            });
        ...
        parser.registerCommand("stats-async", "Показать статистику системы в фоновом потоке", (scanner, system) -> {
            ...
            Future<String> future = system.getBackgroundExecutor().submit(system::generateStatistics);
