...
public class CommandRegistry {
    ...
    private static void registerRoleCommands(CommandParser parser) {
        ...
        parser.registerCommand("role-create", "Создать новую роль", (scanner, system) -> {
            ...
            Role newRole = new Role(id, name, desc);
            try {
    ...
    private static void registerAssignmentCommands(CommandParser parser) {
        ...
        parser.registerCommand("assign-role", "Назначить роль пользователю", (scanner, system) -> {
            ...
            System.out.print("Выбор роли: "); String rn = scanner.nextLine();
            var user = system.getUserManager().findByUsername(un);
            var role = system.getRoleManager().findByName(rn);
            
            if (user.isEmpty() || role.isEmpty()) {
                System.out.println("Ошибка: Пользователь или роль не найдены."); return;
            }
            
            System.out.print("Тип (1 - Постоянное, 2 - Временное): "); String type = scanner.nextLine();
            System.out.print("Причина назначения: "); String reason = scanner.nextLine();
            String id = "ASG-" + System.currentTimeMillis();
            
            try {
                if (type.equals("1")) {
                    system.getAssignmentManager().add(new PermanentAssignment(id, user.get(), role.get(), reason));
                } else {
                    System.out.print("Дата истечения (YYYY-MM-DD): "); String date = scanner.nextLine();
                    system.getAssignmentManager().add(new TemporaryAssignment(id, user.get(), role.get(), date));
                }
                System.out.println("Успех: Роль назначена.");
            } catch (Exception e) {
